...
public class BuySendSwapActivity extends BraveWalletBaseActivity
                ...
                mJsonRpcService.getAllNetworks(CoinType.ETH, chains -> {
                    ...
                    mJsonRpcService.setNetwork(chainId, CoinType.ETH, (success) -> {
                        ...
                        mJsonRpcService.getNetwork(CoinType.ETH, selectedNetwork -> {
                            ...
                            Utils.isCustomNetwork(mJsonRpcService, CoinType.ETH, selectedNetwork.chainId, isCustomNetwork -> {
                ...
                mBlockchainRegistry.getTokenBySymbol(BraveWalletConstants.MAINNET_CHAIN_ID,
                        CoinType.ETH, swapFromAssetSymbol, token -> {
                            ...
                            TokenUtils.getAllTokensFiltered(mBraveWalletService,
                                    mBlockchainRegistry, mCurrentSelectedNetwork.chainId,
    ...
    private void updateBalance(String address, boolean from) {
        ...
        if (blockchainToken == null || blockchainToken.contractAddress.isEmpty()) {
            mJsonRpcService.getBalance(
                    address, CoinType.ETH, mCurrentSelectedNetwork.chainId, (balance, error, errorMessage) -> {
                        warnWhenError(TAG, "getBalance", error, errorMessage);
                        if (error != ProviderError.SUCCESS) {
                            return;
                        }
    ...
    private void adjustControls() {
        ...
        mFromAssetText.setOnClickListener(v -> {
            ...
            bottomSheetDialogFragment.setChainId(mCurrentSelectedNetwork.chainId, mIsCustomNetwork);
    ...
    private class FilterTextWatcherSendToAddr implements TextWatcher {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            mValidator.validate(mCurrentSelectedNetwork.chainId, getKeyringService(), getBlockchainRegistry(),
    ...
    public void updateBuySendSwapAsset(
        ...
        token.contractAddress =
                Utils.getContractAddress(mCurrentSelectedNetwork.chainId, token.symbol, token.contractAddress);
            ...
            mJsonRpcService.getNetwork(CoinType.ETH, network -> {
                ...
                mJsonRpcService.getAllNetworks(CoinType.ETH, chains -> {
                    mJsonRpcService.getCustomNetworks(CoinType.ETH, customNetworkChainIds -> {
                        mIsCustomNetwork = Arrays.asList(customNetworkChainIds).contains(network.chainId);
                        ...
                        for (NetworkInfo chain : chains) {
                            if (network.chainId.equals(chain.chainId)) {
                                ...
                                if (Arrays.asList(customNetworkChainIds).contains(network.chainId)) {

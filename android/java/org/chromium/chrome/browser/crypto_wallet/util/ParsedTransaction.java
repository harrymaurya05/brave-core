...
/*
  * Transaction parser. Java version of components/brave_wallet_ui/common/hooks/transaction-parser.ts.
  */
public class ParsedTransaction extends ParsedTransactionFees {
    ...
    private ParsedTransaction(ParsedTransactionFees parsedTransactionFees) {
        super(parsedTransactionFees.getGasLimit(), parsedTransactionFees.getGasPrice(),
            parsedTransactionFees.getMaxPriorityFeePerGas(), parsedTransactionFees.getMaxFeePerGas(),
            parsedTransactionFees.getGasFee(),  parsedTransactionFees.getGasFeeFiat(),
            parsedTransactionFees.getIsEIP1559Transaction(), parsedTransactionFees.getMissingGasLimitError(),
    ...
    private static BlockchainToken findToken(BlockchainToken[] fullTokenList, String contractAddress) {
        if (contractAddress == null) return null;

        for (BlockchainToken token : fullTokenList) {
            if (token.contractAddress.toLowerCase(Locale.getDefault()).equals(contractAddress.toLowerCase(Locale.getDefault())))
                return token;
        }
        return null;
    }
    ...
    public static ParsedTransaction parseTransaction(TransactionInfo txInfo, NetworkInfo selectedNetwork,
            AccountInfo[] accounts, HashMap<String, Double> assetPrices, BigInteger solFeeEstimatesFee,
        ...
        Double networkSpotPrice = Utils.getOrDefault(assetPrices, selectedNetwork.symbol.toLowerCase(Locale.getDefault()), 0.0d);

        final ParsedTransactionFees feeDetails = ParsedTransactionFees.parseTransactionFees(txInfo, selectedNetwork, networkSpotPrice, solFeeEstimatesFee);

        TxDataUnion txDataUnion = txInfo.txDataUnion;
        TxData1559 txData = txDataUnion.getEthTxData1559();
        SolanaTxData solTxData = txDataUnion.getSolanaTxData();
        ...
        final String value = isSPLTransaction ? solTxData != null ? String.valueOf(solTxData.amount) : ""
        ...
        final String to = isSolTransaction ? solTxData != null ? solTxData.toWalletAddress : ""
            : isFilTransaction ? filTxData.to : txData.baseData.to;

        final String nonce = txData != null ? txData.baseData.nonce : "";
        AccountInfo account = Utils.findAccount(accounts, txInfo.fromAddress);

        final String accountAddressLower = account != null ? account.address.toLowerCase(Locale.getDefault()) : "";
        final BlockchainToken token = isSPLTransaction ? findToken(fullTokenList, solTxData != null ? solTxData.splTokenMintAddress : "") : findToken(fullTokenList, to);
        final String tokenSymbolLower  = token != null ? token.symbol.toLowerCase(Locale.getDefault()) : "";
        final double accountNativeBalance = Utils.getOrDefault(nativeAssetsBalances, accountAddressLower, 0.0d);
        final double accountTokenBalance = Utils.getOrDefault(
            Utils.getOrDefault(blockchainTokensBalances, accountAddressLower, new HashMap<String, Double>()),
        ...
        parsedTransaction.senderLabel = getAddressLabel(account, txInfo.fromAddress);
        ...
        if (txInfo.txType == TransactionType.ERC20_TRANSFER &&
            ...
            final boolean insufficientNativeFunds = parsedTransaction.getGasFee() > accountNativeBalance;
            final boolean insufficientTokenFunds = sendAmount > accountTokenBalance;

            parsedTransaction.recipient = address;
            parsedTransaction.recipientLabel = getAddressLabel(account, address);
            ...
            parsedTransaction.contractAddressError = checkForContractAddressError(fullTokenList, address);
            parsedTransaction.sameAddressError = checkForSameAddressError(address, txInfo.fromAddress);
            ...
            parsedTransaction.recipientLabel = getAddressLabel(account, toAddress);
            ...
            parsedTransaction.contractAddressError = checkForContractAddressError(fullTokenList, toAddress);
            ...
            final int decimals = token != null ? token.decimals : Utils.ETH_DEFAULT_DECIMALS;
            final double totalAmountFiat = parsedTransaction.getGasFeeFiat();
            final BigInteger amountWrapped = new BigInteger(amount, 16);
            final boolean insufficientNativeFunds = parsedTransaction.getGasFee() > accountNativeBalance;

            parsedTransaction.recipient = to;
            parsedTransaction.recipientLabel = getAddressLabel(account, to);
            ...
            parsedTransaction.approvalTargetLabel = getAddressLabel(account, address);
            ...
            final boolean insufficientNativeFunds = parsedTransaction.getGasFee() > accountNativeBalance;
            final boolean insufficientTokenFunds = Utils.fromHexWei(value, decimals) > accountTokenBalance;
            ...
            parsedTransaction.insufficientFundsError = insufficientTokenFunds;
            parsedTransaction.insufficientFundsForGasError = insufficientNativeFunds;
            parsedTransaction.contractAddressError =  checkForContractAddressError(fullTokenList, solTxData != null ? solTxData.toWalletAddress : "");
            parsedTransaction.sameAddressError = checkForSameAddressError(solTxData != null ? solTxData.toWalletAddress : "", txInfo.fromAddress);
        } else if (txInfo.txType == TransactionType.ETH_SWAP &&
                txInfo.txArgs.length > 2) {
            ...
            while (matcher.find()) {
                ...
                fillTokensList.add(address.equals(Utils.ETHEREUM_CONTRACT_FOR_SWAP)
                    ? nativeAsset
                    : thisToken != null ? thisToken : nativeAsset);
            }
            BlockchainToken[] fillTokens = fillTokensList.toArray(new BlockchainToken[0]);

            final BlockchainToken sellToken = fillTokens.length == 1
                ? nativeAsset
                : fillTokens[0];
            final double price = Utils.getOrDefault(assetPrices, tokenSymbolLower, 0.0d);
            final String sellAmountWei = sellAmountArg != null && !sellAmountArg.isEmpty() ? sellAmountArg : value;
            final double sellAmountFiat = Utils.fromHexWei(
                sellAmountWei,
                sellToken.decimals) * price;
            ...
            final double sellTokenBalance = Utils.getOrDefault(
                Utils.getOrDefault(blockchainTokensBalances, accountAddressLower, new HashMap<String, Double>()),
            ...
            final double price = Utils.getOrDefault(assetPrices,
                selectedNetwork.symbol.toLowerCase(Locale.getDefault()), 0.0d);
            ...
            parsedTransaction.insufficientFundsError = parsedTransaction.value + parsedTransaction.getGasFee() > accountNativeBalance;
            parsedTransaction.insufficientFundsForGasError = parsedTransaction.getGasFee() > accountNativeBalance;
            parsedTransaction.isSwap = to.toLowerCase(Locale.getDefault()).equals(Utils.SWAP_EXCHANGE_PROXY);
        }

        return parsedTransaction;
    }

    private static String getAddressLabel(AccountInfo account, String address) {
    ...
    public Pair<String, String> makeTxListItemTitles(Context context) {
        final DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd hh:mm a", Locale.getDefault());
        ...
        if (this.type == TransactionType.ERC20_TRANSFER) {
            ...
            action = String.format(context.getResources().getString(R.string.wallet_tx_info_sent),
                    this.senderLabel, this.formatValueToDisplay(), this.symbol, actionFiatValue, strDate);
            ...
            action = String.format(
                    context.getResources().getString(R.string.wallet_tx_info_approved), this.senderLabel,
                    this.symbol, strDate);
            ...
            detailInfo =
                    String.format(Locale.getDefault(), "%.4f", this.value)
                    + " ETH -> "
                    + "0x Exchange Proxy";
        } else {
            action = String.format(context.getResources().getString(R.string.wallet_tx_info_sent),

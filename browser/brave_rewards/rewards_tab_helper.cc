...
#if BUILDFLAG(ENABLE_IPFS)
  auto ipns_url = GetWebContents().GetLastCommittedURL();
...
bool RewardsTabHelper::BrowserHasWebContents(Browser* browser) {
  int index =
      browser->tab_strip_model()->GetIndexOfWebContents(&GetWebContents());
...
void RewardsTabHelper::MaybeSavePublisherInfo() {
  ...
  // populated as the user nativates the web. Previously, this was accomplished
  // within the background script of the Rewards extension.

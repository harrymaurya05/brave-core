...
// static
std::string KeyringService::GetDefaultKeyringIdForCoin(mojom::CoinType coin) {
...
void KeyringService::AddAccount(const std::string& account_name,
                                mojom::CoinType coin,
                                AddAccountCallback callback) {
  std::string keyring_id = GetDefaultKeyringIdForCoin(coin);
  if (isFilecoinKeyringId(keyring_id)) {
    if (!IsFilecoinEnabled()) {
      std::move(callback).Run(false);
      return;
...
absl::optional<std::string> KeyringService::FindImportedFilecoinKeyring(
...
absl::optional<std::string> KeyringService::FindBasicFilecoinKeyring(
    const std::string& address) const {
  HDKeyring* mainnet_keyring = GetHDKeyringById(mojom::kFilecoinKeyringId);
  HDKeyring* testnet_keyring =
      GetHDKeyringById(mojom::kFilecoinTestnetKeyringId);
...
absl::optional<std::string> KeyringService::FindHardwareFilecoinKeyring(
    const std::string& address) const {
...
absl::optional<std::string> KeyringService::FindFilecoinKeyring(
    const std::string& address) const {
  auto imported = FindImportedFilecoinKeyring(address);
  if (imported) {
    return imported;
  }

  auto basic = FindBasicFilecoinKeyring(address);
  if (basic) {
    return basic;
  }

  auto hardware = FindHardwareFilecoinKeyring(address);
  if (hardware) {
    return hardware;
  }
...
std::string KeyringService::GetImportedKeyringId(
  ...
  return coin_type == mojom::CoinType::FIL
             ? FindImportedFilecoinKeyring(address).value_or(kKeyringNotFound)
             : GetDefaultKeyringIdForCoin(coin_type);
}

std::string KeyringService::GetBasicKeyringId(
    mojom::CoinType coin_type,
    const std::string& address) const {
  return coin_type == mojom::CoinType::FIL
             ? FindBasicFilecoinKeyring(address).value_or(kKeyringNotFound)
             : GetDefaultKeyringIdForCoin(coin_type);
}

std::string KeyringService::GetHardwareKeyring(
    mojom::CoinType coin_type,
    const std::string& address) const {
  return coin_type == mojom::CoinType::FIL
             ? FindHardwareFilecoinKeyring(address).value_or(kKeyringNotFound)
             : GetDefaultKeyringIdForCoin(coin_type);
}
...
std::string KeyringService::GetKeyringId(mojom::CoinType coin_type,
                                         const std::string& address) const {
  return coin_type == mojom::CoinType::FIL
             ? FindFilecoinKeyring(address).value_or(kKeyringNotFound)
             : GetDefaultKeyringIdForCoin(coin_type);
}

std::string KeyringService::GetKeyringForNetwork(
    mojom::CoinType coin_type,
...
void KeyringService::AddHardwareAccounts(
  ...
  for (const auto& info : infos) {
    ...
    auto keyring_id =
        GetKeyringForNetwork(info->coin, info->network.value_or(""));
...
void KeyringService::RemoveHardwareAccount(const std::string& address,
                                           mojom::CoinType coin) {
  auto keyring_id = GetHardwareKeyring(coin, address);
  base::Value* hardware_keyrings =
      GetPrefForKeyringUpdate(prefs_, kHardwareAccounts, keyring_id);
...
absl::optional<std::string> KeyringService::GetSelectedAccount(
  ...
  const base::Value* value = GetPrefForKeyring(
      prefs_, kSelectedAccount, GetDefaultKeyringIdForCoin(coin));
  if (!value)
    return absl::nullopt;
...
bool KeyringService::UpdateNameForHardwareAccountSync(
    ...
    const std::string& name,
    mojom::CoinType coin) {
  auto keyring_id = GetDefaultKeyringIdForCoin(coin);
  base::Value* hardware_keyrings =
      GetPrefForKeyringUpdate(prefs_, kHardwareAccounts, keyring_id);
  for (auto devices : hardware_keyrings->DictItems()) {
    base::Value* account_metas = devices.second.FindKey(kAccountMetas);
